...
```{python selSpp_py}
baseline_WCCTreeValues_selSpp = baseline_WCCTreeValues[baseline_WCCTreeValues["speciesCode"].isin(["BE", "HOL", "POK"])]
baseline_WCCTreeValues_selSpp.head()
```

## R

```{r selSpp_r}
baseline_WCCTreeValues_selSpp <- baseline_WCCTreeValues |>
...
```{python checkMeta_py}
baseline_WCCTreeValues_selSpp[["speciesCode", "spacing", "management"]].drop_duplicates().reset_index()
```

## R

```{r checkMeta_r}
baseline_WCCTreeValues_selSpp |>
...
```{python spman_py}
baseline_WCCTreeValues_selSpp_3_Thinned = baseline_WCCTreeValues_selSpp[(baseline_WCCTreeValues_selSpp.spacing == 3) & (baseline_WCCTreeValues_selSpp.management == "Thinned")]
baseline_WCCTreeValues_selSpp_3_Thinned.head()
```

## R

```{r spman_r}
baseline_WCCTreeValues_selSpp_3_Thinned <- baseline_WCCTreeValues_selSpp |>
...
```{python setDF_py}
df = baseline_WCCTreeValues_selSpp_3_Thinned
