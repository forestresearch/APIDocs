...
```{python url_py}
base_url = "http://209.97.132.245:8000/ESCWCC/"  # Develop
```
...
The object produced (`ESCWCCresponse_dict`) is a nested dictionary with the following structure:
...
* finalYC - The yield class used to retrieve the BCLT data. If the roundedYC is less than the minWCCYC this is set to the minWCCYC. If the roundedYC is greater than the maxWCCYC this is set to thr minWCCYC, else the roundedYC value is used.
